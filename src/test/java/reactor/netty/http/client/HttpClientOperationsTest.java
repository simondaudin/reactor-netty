...
public class HttpClientOperationsTest {
	...
	public void testStatus() {
		...
		HttpClientOperations ops = new HttpClientOperations(() -> channel,
				ConnectionObserver.emptyListener(),
				ClientCookieEncoder.STRICT, ClientCookieDecoder.STRICT);
		ops.setNettyResponse(new DefaultFullHttpResponse(HTTP_1_1,
				new HttpResponseStatus(200, "Some custom reason phrase for 200 status code"), Unpooled.EMPTY_BUFFER));

		assertEquals("OK", ops.status().reasonPhrase());
		assertEquals("Some custom reason phrase for 200 status code", ops.status(true).reasonPhrase());
	}
}
